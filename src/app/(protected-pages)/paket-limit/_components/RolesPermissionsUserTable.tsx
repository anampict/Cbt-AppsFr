        ...
        cell: (props) => {
          ...
          return (
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="font-bold heading-text">{row.name}</div>
                <div>{row.email}</div>
              </div>
            </div>
          );
        ...
        cell: (props) => {
          ...
          return (
            <div className="flex flex-col">
              ...
              <small>{dayjs.unix(row.lastOnline).format("hh:mm A")}</small>
            </div>
          );
        },
      },
              ...
              renderTitle={
                ...
                >
                  <span className="font-bold heading-text">
                    {roleList.find((role) => role.id === row.role)?.name}
                  </span>
                  <TbChevronDown />
                </div>
              }
              ...
              {roleList
                ...
                .map((role) => (
                  <Dropdown.Item
                    key={role.id}
                    eventKey={role.id}
                    onClick={() => handleRoleChange(role.id, row.id)}
                  >
                    {role.name}
                  </Dropdown.Item>
                ))}
